...
const wes = ctxFor('wes', ['europe_only']);
...
console.log('wes    (europe_only)', show(b), '| digest', securityContextDigest(wes));
...
if (b.kind !== 'rows') fail(`europe_only was refused: ${b.why}`);

if (a.regions.length < 2) fail(`analyst should see every territory group, saw ${a.regions.join(', ')}`);
if (b.regions.join() !== 'Europe') fail(`europe_only should see only Europe, saw ${b.regions.join(', ')}`);
